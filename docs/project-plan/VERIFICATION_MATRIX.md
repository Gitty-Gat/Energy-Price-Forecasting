...
| Focused forecast + benchmark verification | `. .venv/bin/activate && python -m unittest tests.test_forecast_pipeline tests.test_benchmark_suite -q` | Verifies forecast and benchmark wrappers avoid irregular-calendar SARIMAX index warnings, exclude leakage-prone exogenous columns, drop constant historical exogenous columns, respect no-exogenous forecast horizons, and emit candidate ablation + parameter-audit artifacts (`Ran 13 tests ... OK`) | Verified |
