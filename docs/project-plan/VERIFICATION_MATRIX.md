...
| Focused forecast + benchmark irregular-index verification | `. .venv/bin/activate && python -m unittest tests.test_forecast_pipeline tests.test_benchmark_suite -q` | Verifies forecast and benchmark wrappers avoid irregular-calendar SARIMAX index warnings while preserving artifact outputs (`Ran 8 tests ... OK`) | Verified |
