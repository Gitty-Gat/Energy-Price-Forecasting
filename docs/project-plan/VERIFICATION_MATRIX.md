...
| Rolling-mean window sensitivity | `. .venv/bin/activate && for window in 5 10 20 60; do python src/diagnostics/benchmark_suite.py --merged data/processed/merged_exog.csv --outputs results/backtests/rolling_mean_window_${window}_integrity_check --horizons 5 20 --eval-step 200 --min-train-size 600 --rolling-mean-window "$window" --candidate-variants sentiment_only no_exogenous; done` | Existing repo data only. Window 20 remains best rolling-mean window for NG 5d/20d; OL leaders remain simple AR 5d and random walk 20d. No test-only candidate became promotion-ready. See `docs/project-plan/ROLLING_MEAN_WINDOW_SENSITIVITY_2026-04-30.md`. | Verified |
| Promotion gate hardening | `. .venv/bin/activate && python -m unittest tests.test_benchmark_suite -q && python src/diagnostics/benchmark_suite.py --merged data/processed/merged_exog.csv --outputs results/backtests/promotion_gate_check_2026-04-30 --horizons 5 20 --eval-step 400 --min-train-size 600 --candidate-variants sentiment_only no_exogenous` | Benchmark now emits `benchmark_candidate_promotion_gates.csv` plus metadata policy. Candidates must pass RMSE, MAE, and uncertainty sanity gates before promotion. Smoke check kept all test-only ARIMAX controls non-promoted. See `docs/project-plan/PROMOTION_GATE_HARDENING_2026-04-30.md`. | Verified |
